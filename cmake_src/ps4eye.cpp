 ...
 *  license.
 *
 *  Modified by Joshua Girard for linux support.
 *
 */
...
#include <algorithm>
#include <ps4/ps4eye.h>
#include <ps4/ps4eye_regs.h>
#include <ps4/libusb.h>
#include <iostream>
#include <fstream>
...
#define ARRAY_SIZE(_A) (sizeof(_A) / sizeof((_A)[0]))
    ...
    static int64_t getTickCount()
    {
		...
		return (int64_t) (time.tv_nsec / 1000L + time.tv_sec * 1000000L); // millisecs
#endif
    }
...
#define UVC_STREAM_FID	(1 << 0)
	...
	while ((dev = devs[i++]) != NULL){
	    struct libusb_device_descriptor desc;
            ...
            if(desc.idVendor == PS4EYECam::VENDOR_ID && (desc.idProduct == PS4EYECam::PRODUCT_ID || desc.idProduct == 0x0580))
		cnt++;
	}
    ...
    public:
        ...
        bool start_transfers(libusb_device_handle *handle, uint32_t curr_frame_size)
        {
			...
			int packet_size = 49152;//32768;//libusb_get_max_iso_packet_size(libusb_get_device(handle), 0x81);

			debug("URB Transfer Packet Size: %d\n",packet_size);

			int transfer_size = num_transfers * packet_size;

            int i;
            struct libusb_transfer *xfr0;

			debug("Alloc urb transfers...\n");
            for(i=0;i<num_transfers;i++)
            {
                xfr[i]=libusb_alloc_transfer(num_transfers);
                //  xfr[i]=xfr0;
...
*/
                ...
                //xfr[i]->flags = LIBUSB_TRANSFER_ADD_ZERO_PACKET;
                 ...
                 xfr[i]->timeout = 1000; // was 10000
                 xfr[i]->buffer = frame_buffer_end+transfer_size*i;
                 xfr[i]->user_data = this;
                 xfr[i]->length = transfer_size; // max size in transfer 49152*8
                 xfr[i]->callback = cb_xfr;
                 xfr[i]->num_iso_packets = num_transfers; //num isoc
        ...
        if (xfr->type == LIBUSB_TRANSFER_TYPE_ISOCHRONOUS) {
            ...
            if(urb->ff71status<0)
            {
                ...
                else
                {
                    ...
                    {
                        urb->pkt_scan(xfr->buffer+49152/*32768*/*i, pack->actual_length);
                    }
                }
    ...
    int PS4EYECam::countDevices(){
	return USBMgr::instance()->countDevices();
    }
    ...
    const std::vector<PS4EYECam::PS4EYERef>& PS4EYECam::getDevices( bool forceRefresh )
    {
		...
		{ 
	//		std::cout<<" The size of the vector is (mode1) "<<devices.size()<<std::endl;
			return devices;
		}
	...
	//	std::cout<<" The size of the vector is (mode 2)"<<devices.size()<<std::endl;
...
*/
    ...
    void PS4EYECam::start()
    {
       ...
       //TODO define different  modes to choose resolution and fps
		...
		debug("starting urb transfers...\n");
        urb->start_transfers(handle_, frame_stride*frame_height);
		debug("urb transfers started\n");
	...
	void PS4EYECam::getLastLeftVideoFrame(uint8_t * buffer){
        ...
        {
            memcpy(&buffer[frame_width*2*i],&rowpointer[linesize*2*i+32+64], frame_width*2);
        }

	}

	void PS4EYECam::getLastRightVideoFrame(uint8_t * buffer){
        int i;
        last_qued_frame_time = urb->last_frame_time;
        ...
        for(i=0;i<frame_height;i++)
        {
             memcpy(&buffer[frame_width*2*i],&rowpointer[linesize*2*i+32+64+frame_width*2], frame_width*2);
        }
	...
	//Quantize time to integer, store millisecond accuracy
	myframe.timestamp = (uint64_t)(last_qued_frame_time * 1000.0);
   	...
   	int PS4EYECam::uvc_set_video_mode(uint8_t mode,uint8_t fps)
	{
		...
		switch(mode)
		{
			...
			case 1://1748x408
				...
				{
					debug("Unsupported fps=%d take default 120 fps\n",fps);
					frame_rate=120;
				}
				else
				{
			...
			case 2://898x200
				...
				{
					debug("Unsupported fps=%d take default 240 fps\n",fps);
					frame_rate=240;
				}
				else
				{
        ...
        if(res != 0) {
			switch (res){
				case LIBUSB_ERROR_NOT_FOUND:
					debug("set configuration error: LIBUSB_ERROR_NOT_FOUND\n");
					break;
				case LIBUSB_ERROR_BUSY:
					debug("set configuration error: LIBUSB_ERROR_BUSY\n");
					break;
				case LIBUSB_ERROR_NO_DEVICE:
					debug("set configuration error: LIBUSB_ERROR_NO_DEVICE\n");
					break;
				default:
					debug("set configuration error: LIBUSB_ERROR: %d\n", res);
					break;
			}
